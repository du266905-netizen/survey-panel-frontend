...
export default function BusinessCustomQuestionnaireRequest() {
  ...
  const canContinue = request.title.trim().length >= 3 && request.researchGoal.trim().length >= 20 && request.audienceDescription.trim().length >= 10;
  ...
  return <main className="business-custom-request-page">
    <div className="business-custom-request-shell">
      ...
      <section className="business-custom-request-surface">
        <header><button type="button" onClick={() => goWorkspace()}><ArrowLeft size={16} /> {copy.back}</button><span>{copy.eyebrow}</span></header>
        <form className="business-custom-request-form" onSubmit={submit}>
          <div className="business-custom-request-heading"><p>{step === 1 ? copy.stepOne : copy.stepTwo}</p><h1>{step === 1 ? copy.stepOneTitle : copy.stepTwoTitle}</h1><span>{step === 1 ? copy.stepOneIntro : copy.stepTwoIntro}</span></div>
          ...
          <footer>{step === 2 && <button type="button" className="business-custom-request-quiet" onClick={() => setStep(1)}>{copy.previous}</button>}{step === 1 ? <button type="button" className="business-button" onClick={() => setStep(2)} disabled={!canContinue}>{copy.continue} <ArrowRight size={16} /></button> : <button className="business-button" type="submit" disabled={submitting}>{submitting ? <LoaderCircle className="animate-spin" size={16} /> : copy.submit} {!submitting && <ArrowRight size={16} />}</button>}</footer>
        </form>
      </section>
    </div>
    {showIntro && <div className="business-custom-intro-modal" role="dialog" aria-modal="true" aria-labelledby="custom-questionnaire-intro-title"><section><button className="business-custom-intro-close" type="button" onClick={() => setShowIntro(false)} aria-label="Close"><X size={20} /></button><div className="business-custom-intro-copy"><p>{copy.modalEyebrow}</p><h2 id="custom-questionnaire-intro-title">{copy.modalTitle}</h2><span>{copy.modalBody}</span><ol>{copy.modalSteps.map((item, index) => <li key={item}><b>{String(index + 1).padStart(2, '0')}</b>{item}</li>)}</ol><div><button className="business-button" type="button" onClick={() => setShowIntro(false)}>{copy.start} <ArrowRight size={16} /></button><button type="button" onClick={() => goWorkspace()}>{copy.later}</button></div></div><figure><img src={serviceImage} alt="" /></figure></section></div>}
  </main>;
}
