...
export default function BusinessCustomQuestionnaireRequest() {
  ...
  const canContinue = request.title.trim().length >= 3;
    ...
    <div className="business-custom-request-shell">
      ...
      <section className="business-custom-request-surface">
        ...
        <form className="business-custom-request-form" onSubmit={submit}>
          ...
          <footer>{step === 2 && <button type="button" className="business-custom-request-quiet" onClick={() => { setError(''); setStep(1); }}>{copy.previous}</button>}{step === 1 ? <button type="button" className="business-button business-custom-request-next" onClick={goNext} disabled={!canContinue}>{copy.continue} <ArrowRight size={16} /></button> : <button className="business-button" type="submit" disabled={submitting}>{submitting ? <LoaderCircle className="animate-spin" size={16} /> : copy.submit} {!submitting && <ArrowRight size={16} />}</button>}</footer>
        </form>
      </section>
    </div>
