...
export default function BusinessQuestionnaireBuilder() {
  ...
  const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [message, setMessage] = useState(''); const [error, setError] = useState(''); const [sensitiveNoticeOpen, setSensitiveNoticeOpen] = useState(false); const [responses, setResponses] = useState([]); const [previewOpen, setPreviewOpen] = useState(false);
  ...
  return <main className="business-builder"><header className="business-builder-header"><Link to="/business/workspace?view=questionnaires"><ArrowLeft size={17} /> Questionnaire editor</Link><div><span>{statusLabel}{isDirty && ' · Unsaved changes'}</span><button type="button" onClick={() => save()} disabled={saving || !isDirty}>{saving ? <LoaderCircle className="animate-spin" size={16} /> : 'Save draft'}</button><details className="business-builder-collection"><summary>Response collection</summary><div><strong>Choose this when you are ready to decide how responses will be collected.</strong>{collectionOptions.map(([value, label, description]) => <label key={value} className={collectionMode === value ? 'is-selected' : ''}><input type="radio" name="collectionMode" disabled={structureLocked} value={value} checked={collectionMode === value} onChange={() => setCollectionMode(value)} /><span><b>{label}</b><small>{description}</small></span></label>)}</div></details><details className="business-builder-guide"><summary>Research design guide</summary><div><strong>Measure one thing at a time.</strong><ul><li>Use screening only to establish eligibility.</li><li>Use rating for intensity and ranking only for forced priorities.</li><li>Define an outcome, then add drivers, controls and segments only when useful.</li><li>Keep profile questions necessary; make sensitive fields optional.</li></ul><span>Drafting guidance only — it does not replace research-method review.</span></div></details><button type="button" onClick={() => { setPreviewOpen(true); setError(''); }} disabled={saving}><Eye size={15} /> Preview</button>{questionnaire.status === 'DRAFT' && responseCount === 0 && <button className="business-builder-delete" type="button" onClick={deleteDraft} disabled={saving}><Trash2 size={15} /> Delete draft</button>}{['DRAFT', 'ARCHIVED'].includes(questionnaire.status) && <button className="business-builder-publish" type="button" onClick={() => save({ publish: true })} disabled={saving || !questions.length}>{saving ? <LoaderCircle className="animate-spin" size={16} /> : collectionMode === 'SELF_DISTRIBUTED' ? 'Submit for publication' : 'Request quote & review'} <ArrowRight size={16} /></button>}{['PUBLISHED', 'PENDING_REVIEW', 'APPROVED'].includes(questionnaire.status) && <button type="button" onClick={archive} disabled={saving}><Archive size={15} /> Archive</button>}</div></header>
    <div className="business-builder-layout"><section className="business-builder-main"><p className="business-eyebrow">QUESTIONNAIRE EDITOR</p><input className="business-builder-title" value={title} onChange={(event) => setTitle(event.target.value)} aria-label="Questionnaire title" placeholder="Questionnaire title" /><details className="business-cover-field"><summary>Respondent introduction <span>Optional</span></summary><label><span>What will respondents know?</span><textarea value={coverDescription} onChange={(event) => setCoverDescription(event.target.value)} maxLength={600} placeholder="Explain the purpose of this questionnaire and how responses will be used." /></label></details><details className="business-audience-modules"><summary><span><b>Audience profile</b><small>Add only the screening or analysis variables you need.</small></span><em><UsersRound size={15} /> Add profile variables</em></summary><div className="business-audience-module-grid">{audienceModules.map((module) => { const alreadyAdded = questions.some((question) => question.section === 'PROFILE' && question.prompt === module.prompt); return <article key={module.id}><div><strong>{module.title}</strong><span>{module.detail}</span></div><button type="button" onClick={() => addAudienceModule(module)} disabled={structureLocked || alreadyAdded}><Plus size={14} /> {alreadyAdded ? 'Added' : 'Add'}</button></article>; })}</div><footer><ShieldAlert size={14} /> Ask only for details needed to screen or analyse; sensitive fields should remain optional.</footer></details>{structureLocked && <div className="business-builder-lock"><strong>This questionnaire already has responses.</strong><span>Question wording and structure are locked to protect the integrity of your data.</span><button type="button" onClick={duplicate} disabled={saving}>Copy as new questionnaire <ArrowRight size={15} /></button></div>}{message && <p className="business-builder-message is-success"><Check size={16} /> {message}</p>}{error && <p className="business-builder-message">{error}</p>}
      <div className="business-add-question"><ClipboardList size={20} /><div><strong>Add a question</strong><span>Choose a measurement method that fits the decision you need to make.</span></div><div className="business-add-question-actions">{questionTypes.map(([type, label]) => <button type="button" disabled={structureLocked} onClick={() => setQuestions((current) => [...current, newQuestion(type)])} key={type}><Plus size={14} /> {label}</button>)}</div></div>
    ...
    </section><aside className="business-builder-sidebar"><section className="business-question-navigator"><p>QUESTION MAP</p><h2>Questions</h2><div>{questions.length ? questions.map((question, index) => <button key={questionIdentity(question)} type="button" onClick={() => document.getElementById(`business-question-${questionIdentity(question)}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })}><span>{index + 1}</span><strong>{question.prompt || 'Untitled question'}</strong><small>{questionSections.find(([value]) => value === question.section)?.[1]}</small></button>) : <span className="business-question-navigator-empty">Add a question to create your map.</span>}</div></section><section className="business-design-checks"><p>DESIGN CHECK</p><h2>Before you publish</h2>{checks.map(([state, text], index) => <div className={state} key={`${state}-${index}`}><span>{state === 'ready' ? <Check size={14} /> : '!'}</span>{text}</div>)}</section></aside></div>
