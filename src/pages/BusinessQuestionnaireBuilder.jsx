...
const questionTypeGuidance = {
  SINGLE_CHOICE: 'Use when answers must be mutually exclusive. Include an appropriate opt-out or “none” option when the audience may not fit any choice.',
  MULTIPLE_CHOICE: 'Use when more than one answer can be true. Set a maximum when it reflects a real limit.',
  RANKING: 'Use only when respondents must trade off priorities; it is not a substitute for asking how strongly each item is felt.',
  RATING: 'Use one construct per scale and keep the direction consistent across the questionnaire. Label both ends when labels are needed.',
  NUMBER: 'Use for a quantity the respondent can reasonably recall or estimate; state the unit in the question.',
  SHORT_TEXT: 'Use for a brief factual answer that does not require coding a long explanation.',
  LONG_TEXT: 'Use sparingly for explanations or unanticipated ideas; open answers take longer to answer and analyse.',
};
...
function QuestionCard({ question, index, questions, locked, onUpdate, onRemove, onMove }) {
  ...
  return <article id={`business-question-${questionIdentity(question)}`} className="business-question-card business-question-card-rich">
    ...
    <details className="business-question-variable"><summary>Research variable <span>{settings.measurementRole === 'OUTCOME' ? 'Outcome' : settings.measurementRole.replaceAll('_', ' ').toLowerCase()}</span></summary><p>Label the role this question plays in the study. This helps keep the questionnaire focused; it does not claim causation.</p><div><label>Role<select disabled={locked} value={settings.measurementRole} onChange={(event) => updateSettings({ measurementRole: event.target.value })}><option value="OUTCOME">Outcome / dependent variable</option><option value="DRIVER">Driver / explanatory variable</option><option value="SCREENING">Eligibility screening variable</option><option value="SEGMENT">Profile or segmentation variable</option><option value="CONTROL">Control or comparison variable</option><option value="EXPLORATORY">Exploratory question</option></select></label><label>Variable label <input disabled={locked} value={settings.variableName} onChange={(event) => updateSettings({ variableName: event.target.value })} placeholder="e.g. Purchase intention" /></label><label>Reference period <input disabled={locked} value={settings.referencePeriod} onChange={(event) => updateSettings({ referencePeriod: event.target.value })} placeholder="e.g. Past 30 days" /></label></div></details>
...
function buildChecks(questions) {
  ...
  if (core && !outcomes) checks.push(['warning', 'Mark at least one core question as an outcome variable so the study has a clear decision measure.']);
  ...
  questions.forEach((question, index) => {
    ...
    if (question.type === 'RATING' && Boolean(question.settings?.scaleLowLabel) !== Boolean(question.settings?.scaleHighLabel)) checks.push(['warning', `Question ${index + 1} needs labels at both ends of its scale, or neither.`]);
    ...
    if (question.logic?.sourcePosition && question.logic.sourcePosition < index + 1) {
      const source = questions[question.logic.sourcePosition - 1];
      if (!source || !logicSourceQuestion(source.type) || !source.choices?.includes(question.logic.value)) checks.push(['warning', `Question ${index + 1} has a display rule that no longer matches its source answer.`]);
    }
    if (/\b(and|or)\b|以及|同时|和.*和/i.test(question.prompt)) checks.push(['warning', `Question ${index + 1} may ask about more than one thing. Check that one answer can be interpreted clearly.`]);
  });
...
export default function BusinessQuestionnaireBuilder() {
  ...
  const statusLabel = questionnaire?.status === 'PENDING_REVIEW' ? 'Audience and questionnaire review pending' : questionnaire?.status === 'APPROVED' ? 'Research scope approved · awaiting launch' : questionnaire?.status === 'PUBLISHED' ? (questionnaire.isCollecting ? 'Live' : 'Collection paused') : questionnaire?.status === 'ARCHIVED' ? 'Archived' : 'Draft';
  ...
  return <main className="business-builder"><header className="business-builder-header"><Link to="/business/workspace?view=questionnaires"><img className="business-builder-brand-mark" src="/guanyisearch-brand-mark-transparent.png" alt="GuanyiSearch" /> <ArrowLeft size={17} /> Questionnaire editor</Link><div><span>{statusLabel}{isDirty && ' · Unsaved changes'}</span><button type="button" onClick={() => save()} disabled={saving || !isDirty}>{saving ? <LoaderCircle className="animate-spin" size={16} /> : 'Save draft'}</button><span className="business-builder-audience-status"><UsersRound size={14} /> Audience plan ready</span><details className="business-builder-guide"><summary>Research design guide</summary><div><strong>Measure one thing at a time.</strong><ul><li>Use screening only to establish eligibility.</li><li>Use rating for intensity and ranking only for forced priorities.</li><li>Define an outcome, then add drivers, controls and segments only when useful.</li><li>Keep profile questions necessary; make sensitive fields optional.</li></ul><span>Drafting guidance only — it does not replace research-method review.</span></div></details><button type="button" onClick={() => { setPreviewOpen(true); setError(''); }} disabled={saving}><Eye size={15} /> Preview</button>{questionnaire.status === 'DRAFT' && responseCount === 0 && <button className="business-builder-delete" type="button" onClick={deleteDraft} disabled={saving}><Trash2 size={15} /> Delete draft</button>}{['DRAFT', 'ARCHIVED'].includes(questionnaire.status) && <button className="business-builder-publish" type="button" onClick={() => save({ publish: true })} disabled={saving || !questions.length}>{saving ? <LoaderCircle className="animate-spin" size={16} /> : 'Request feasibility & review'} <ArrowRight size={16} /></button>}{['PUBLISHED', 'PENDING_REVIEW', 'APPROVED'].includes(questionnaire.status) && <button type="button" onClick={archive} disabled={saving}><Archive size={15} /> Archive</button>}</div></header>
    <div className="business-builder-layout"><section className="business-builder-main"><p className="business-eyebrow">QUESTIONNAIRE EDITOR</p><input className="business-builder-title" value={title} onChange={(event) => setTitle(event.target.value)} aria-label="Questionnaire title" placeholder="Questionnaire title" /><details className="business-study-brief"><summary><span><b>Research brief</b><small>{project.researchGoal || 'Add the decision this questionnaire should support.'}</small></span><em>{questions.length} questions · {variableCounts.OUTCOME || 0} outcome{variableCounts.OUTCOME === 1 ? '' : 's'}</em></summary><dl><div><dt>Decision</dt><dd>{project.researchGoal || 'Not set'}</dd></div><div><dt>Audience</dt><dd>{project.audienceDescription || 'Not set'}</dd></div>{project.countries && <div><dt>Market</dt><dd>{project.countries}</dd></div>}{project.estimatedMinutes && <div><dt>Estimated length</dt><dd>{project.estimatedMinutes} min</dd></div>}<div><dt>Variables</dt><dd>{variableCounts.OUTCOME || 0} outcomes · {variableCounts.DRIVER || 0} drivers · {variableCounts.SEGMENT || 0} segments · {variableCounts.SCREENING || 0} screening</dd></div></dl></details><details className="business-cover-field"><summary>Respondent introduction <span>Optional</span></summary><label><span>What will respondents know?</span><textarea value={coverDescription} onChange={(event) => setCoverDescription(event.target.value)} maxLength={600} placeholder="Explain the purpose of this questionnaire and how responses will be used." /></label></details><details className="business-audience-modules"><summary><span><b>Audience profile</b><small>Add only the screening or analysis variables you need.</small></span><em><UsersRound size={15} /> Add profile variables</em></summary><div className="business-audience-module-grid">{audienceModules.map((module) => { const alreadyAdded = questions.some((question) => question.section === 'PROFILE' && question.prompt === module.prompt); return <article key={module.id}><div><strong>{module.title}</strong><span>{module.detail}</span></div><button type="button" onClick={() => addAudienceModule(module)} disabled={structureLocked || alreadyAdded}><Plus size={14} /> {alreadyAdded ? 'Added' : 'Add'}</button></article>; })}</div><footer><ShieldAlert size={14} /> Ask only for details needed to screen or analyse; sensitive fields should remain optional.</footer></details>{structureLocked && <div className="business-builder-lock"><strong>This questionnaire already has responses.</strong><span>Question wording and structure are locked to protect the integrity of your data.</span><button type="button" onClick={duplicate} disabled={saving}>Copy as new questionnaire <ArrowRight size={15} /></button></div>}{message && <p className="business-builder-message is-success"><Check size={16} /> {message}</p>}{error && <p className="business-builder-message">{error}</p>}
      <section className="business-question-toolbar"><div><ClipboardList size={19} /><span><strong>Add a question</strong><small>Choose one measurement method, then add it to the end of this draft.</small></span></div><label><span>Measurement method</span><select value={newQuestionType} disabled={structureLocked} onChange={(event) => setNewQuestionType(event.target.value)}>{questionTypes.map(([type, label]) => <option value={type} key={type}>{label}</option>)}</select></label><button type="button" className="business-builder-publish" disabled={structureLocked} onClick={() => setQuestions((current) => [...current, newQuestion(newQuestionType)])}><Plus size={15} /> Add question</button></section>
    ...
    </section><aside className="business-builder-sidebar"><section className="business-question-navigator"><p>QUESTION MAP</p><h2>Questions</h2><div>{questions.length ? questions.map((question, index) => <button key={questionIdentity(question)} type="button" onClick={() => document.getElementById(`business-question-${questionIdentity(question)}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })}><span>{index + 1}</span><strong>{question.prompt || 'Untitled question'}</strong><small>{questionSections.find(([value]) => value === question.section)?.[1]}</small>{question.logic && <i>Shown after Q{question.logic.sourcePosition}: {question.logic.value}</i>}</button>) : <span className="business-question-navigator-empty">Add a question to create your map.</span>}</div></section><section className="business-design-checks"><p>DESIGN CHECK</p><h2>Before you publish</h2>{checks.map(([state, text], index) => <div className={state} key={`${state}-${index}`}><span>{state === 'ready' ? <Check size={14} /> : '!'}</span>{text}</div>)}</section></aside></div>
