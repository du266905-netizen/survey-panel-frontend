...
const collectionOptions = [
  ...
  ['AUDIENCE_SOURCING', 'Source respondents', 'Discuss a target audience and response plan with sales.'],
  ['RESEARCH_SUPPORT', 'Research support', 'Discuss questionnaire design or a tailored study with sales.'],
];
...
export default function BusinessQuestionnaireBuilder() {
  ...
  const statusLabel = questionnaire?.status === 'PENDING_REVIEW' ? 'Pending publication' : questionnaire?.status === 'PUBLISHED' ? (questionnaire.isCollecting ? 'Live' : 'Collection paused') : questionnaire?.status === 'ARCHIVED' ? 'Archived' : 'Draft';
  ...
  const save = async ({ publish = false, sensitiveDataAcknowledged = false } = {}) => {
    ...
    try {
      ...
      setProject((current) => ({ ...current, questionnaire: nextQuestionnaire })); setQuestions(nextQuestionnaire.questions || questions); setSensitiveNoticeOpen(false); setMessage(publish ? 'Your questionnaire is pending publication.' : 'Draft saved.');
    } catch (caughtError) {
  ...
  return <main className="business-builder">
    <header className="business-builder-header"><Link to="/business/workspace"><ArrowLeft size={17} /> Projects</Link><div><span>{statusLabel}</span><button type="button" onClick={() => save()} disabled={saving}>{saving ? <LoaderCircle className="animate-spin" size={16} /> : 'Save draft'}</button>{['DRAFT', 'ARCHIVED'].includes(questionnaire.status) && <button className="business-builder-publish" type="button" onClick={() => save({ publish: true })} disabled={saving || !questions.length}>{saving ? <LoaderCircle className="animate-spin" size={16} /> : 'Request publication'} <ArrowRight size={16} /></button>}{['PUBLISHED', 'PENDING_REVIEW'].includes(questionnaire.status) && <button type="button" onClick={archive} disabled={saving}><Archive size={15} /> Archive</button>}</div></header>
    <div className="business-builder-layout"><section className="business-builder-main"><p className="business-eyebrow">QUESTIONNAIRE</p><input className="business-builder-title" value={title} onChange={(event) => setTitle(event.target.value)} aria-label="Questionnaire title" placeholder="Questionnaire title" /><p className="business-builder-lede">Build your questions, describe the purpose for respondents, then request publication when you are ready to share.</p><label className="business-cover-field"><span>What will respondents know?</span><textarea value={coverDescription} onChange={(event) => setCoverDescription(event.target.value)} maxLength={600} placeholder="Explain the purpose of this questionnaire and how the responses will be used." /></label>{structureLocked && <div className="business-builder-lock"><strong>This questionnaire already has responses.</strong><span>Question types, wording, options and required settings are locked to protect the integrity of your data.</span><button type="button" onClick={duplicate} disabled={saving}>Copy as new questionnaire <ArrowRight size={15} /></button></div>}{message && <p className="business-builder-message is-success"><Check size={16} /> {message}</p>}{error && <p className="business-builder-message">{error}</p>}
      ...
      <aside className="business-builder-sidebar"><section><p>RESPONSE COLLECTION</p><h2>How will you collect responses?</h2><div className="business-collection-options">{collectionOptions.map(([value, label, description]) => <label key={value} className={collectionMode === value ? 'is-selected' : ''}><input type="radio" name="collectionMode" disabled={structureLocked} value={value} checked={collectionMode === value} onChange={() => setCollectionMode(value)} /><strong>{label}</strong><span>{description}</span></label>)}</div>{collectionMode !== 'SELF_DISTRIBUTED' && <Link className="business-builder-sales" to="/business/access">Contact sales <ArrowRight size={15} /></Link>}</section><section className="business-share-card"><p>SHARE LINK</p>{questionnaire.status === 'PUBLISHED' && questionnaire.isCollecting ? <><strong>Ready to send</strong><span>{publicUrl}</span><button type="button" onClick={copyPublicLink}><Copy size={15} /> Copy link</button><a href={publicUrl} target="_blank" rel="noreferrer">Open questionnaire <ArrowRight size={15} /></a></> : questionnaire.status === 'PENDING_REVIEW' ? <><strong>Pending publication</strong><span>Your link will be available when publication is complete.</span></> : questionnaire.status === 'ARCHIVED' ? <><strong>Archived</strong><span>This public link no longer accepts responses. Copy the questionnaire to create a new version.</span></> : <><strong>Request publication to create a shareable link.</strong><span>Your questions stay in draft until then.</span></>}</section><section className="business-response-card"><p>RESPONSES</p><strong>{responseCount}</strong><span>responses received</span></section></aside></div>
