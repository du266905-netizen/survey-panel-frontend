...
function workspaceGreeting(language, returningUser) {
  const returning = {
    'zh-CN': '欢迎回来，', 'zh-Hant': '歡迎回來，', de: 'Willkommen zurück,', fr: 'Bon retour,',
    es: 'Bienvenido de nuevo,', it: 'Bentornato,', nl: 'Welkom terug,', da: 'Velkommen tilbage,',
    fi: 'Tervetuloa takaisin,', no: 'Velkommen tilbake,', pt: 'Bem-vindo de volta,', ru: 'С возвращением,',
    sv: 'Välkommen tillbaka,', tr: 'Tekrar hoş geldin,', ko: '다시 오신 것을 환영합니다,', ja: 'おかえりなさい、',
  };
  const firstVisit = {
    'zh-CN': ['早上好，', '下午好，', '晚上好，'], 'zh-Hant': ['早安，', '午安，', '晚安，'],
    de: ['Guten Morgen,', 'Guten Tag,', 'Guten Abend,'], fr: ['Bonjour,', 'Bon après-midi,', 'Bonsoir,'],
    es: ['Buenos días,', 'Buenas tardes,', 'Buenas noches,'], it: ['Buongiorno,', 'Buon pomeriggio,', 'Buonasera,'],
    nl: ['Goedemorgen,', 'Goedemiddag,', 'Goedenavond,'], da: ['Godmorgen,', 'God eftermiddag,', 'Godaften,'],
    fi: ['Hyvää huomenta,', 'Hyvää iltapäivää,', 'Hyvää iltaa,'], no: ['God morgen,', 'God ettermiddag,', 'God kveld,'],
    pt: ['Bom dia,', 'Boa tarde,', 'Boa noite,'], ru: ['Доброе утро,', 'Добрый день,', 'Добрый вечер,'],
    sv: ['God morgon,', 'God eftermiddag,', 'God kväll,'], tr: ['Günaydın,', 'Tünaydın,', 'İyi akşamlar,'],
    ko: ['좋은 아침이에요,', '좋은 오후예요,', '좋은 저녁이에요,'], ja: ['おはようございます、', 'こんにちは、', 'こんばんは、'],
  };
  if (returningUser) return returning[language] || 'Welcome back,';
  const hour = new Date().getHours();
  const labels = firstVisit[language] || ['Good morning,', 'Good afternoon,', 'Good evening,'];
  return hour < 12 ? labels[0] : hour < 18 ? labels[1] : labels[2];
}

const workspaceHomeText = {
  'en-US': ['Start with a research goal, then organise projects, questionnaires, and real responses.', 'Start new research', 'Choose a custom questionnaire or tailored study', 'Start with AI', 'Start a research brief in a conversation', 'Questionnaire editor', 'Find your audience and plan questionnaire research', 'Research overview', 'Only real records from this workspace.', 'View results', 'Projects', 'Questionnaire drafts', 'In planning', 'Responses', 'Recent projects', 'Continue work already in progress.', 'All projects', 'Prepare your first research brief.'],
  'zh-CN': ['从一个研究目标开始，组织项目、问卷与真实答卷。', '新建研究', '选择定制问卷或定制研究', '用 AI 开始', '用对话开始一份研究简报', '问卷编辑器', '寻找受众群体，并规划问卷研究', '研究概览', '只显示此工作区的真实记录。', '查看结果', '项目', '问卷草稿', '待确认范围', '已收答卷', '最近项目', '继续处理你的研究工作。', '所有项目', '准备第一份研究简报。'],
  'zh-Hant': ['從一個研究目標開始，整理專案、問卷與真實答卷。', '開始新研究', '選擇客製問卷或專屬研究', '使用 AI 開始', '透過對話開始一份研究簡報', '問卷編輯器', '尋找目標對象並規劃問卷研究', '研究概覽', '只顯示此工作區的真實紀錄。', '查看結果', '專案', '問卷草稿', '待確認範圍', '已收答卷', '最近專案', '繼續處理你的研究工作。', '所有專案', '準備第一份研究簡報。'],
  de: ['Beginnen Sie mit einem Forschungsziel und organisieren Sie Projekte, Fragebögen und echte Antworten.', 'Neue Forschung starten', 'Individuellen Fragebogen oder Studie wählen', 'Mit KI starten', 'Ein Forschungsbriefing im Dialog beginnen', 'Fragebogen-Editor', 'Zielgruppe finden und Fragebogenforschung planen', 'Forschungsübersicht', 'Nur echte Datensätze aus diesem Arbeitsbereich.', 'Ergebnisse ansehen', 'Projekte', 'Fragebogenentwürfe', 'In Planung', 'Antworten', 'Letzte Projekte', 'Laufende Forschung fortsetzen.', 'Alle Projekte', 'Erstes Forschungsbriefing vorbereiten.'],
  fr: ['Commencez par un objectif de recherche, puis organisez projets, questionnaires et réponses réelles.', 'Nouvelle recherche', 'Choisir un questionnaire ou une étude sur mesure', 'Commencer avec l’IA', 'Démarrer un brief de recherche en conversation', 'Éditeur de questionnaire', 'Trouver votre audience et planifier l’étude', 'Vue d’ensemble', 'Uniquement les données réelles de cet espace.', 'Voir les résultats', 'Projets', 'Brouillons de questionnaires', 'À planifier', 'Réponses', 'Projets récents', 'Reprendre une recherche en cours.', 'Tous les projets', 'Préparer votre premier brief.'],
  es: ['Empieza con un objetivo de investigación y organiza proyectos, cuestionarios y respuestas reales.', 'Nueva investigación', 'Elegir un cuestionario o estudio personalizado', 'Empezar con IA', 'Iniciar un brief de investigación en una conversación', 'Editor de cuestionarios', 'Encuentra tu audiencia y planifica la investigación', 'Resumen de investigación', 'Solo registros reales de este espacio de trabajo.', 'Ver resultados', 'Proyectos', 'Borradores de cuestionarios', 'En planificación', 'Respuestas', 'Proyectos recientes', 'Continúa una investigación en curso.', 'Todos los proyectos', 'Prepara tu primer brief.'],
  it: ['Inizia da un obiettivo di ricerca e organizza progetti, questionari e risposte reali.', 'Nuova ricerca', 'Scegli un questionario o uno studio personalizzato', 'Inizia con l’IA', 'Avvia un brief di ricerca in una conversazione', 'Editor di questionari', 'Trova il tuo pubblico e pianifica la ricerca', 'Panoramica della ricerca', 'Solo dati reali di questo spazio di lavoro.', 'Vedi risultati', 'Progetti', 'Bozze di questionari', 'In pianificazione', 'Risposte', 'Progetti recenti', 'Riprendi una ricerca in corso.', 'Tutti i progetti', 'Prepara il tuo primo brief.'],
  nl: ['Begin met een onderzoeksdoel en organiseer projecten, vragenlijsten en echte antwoorden.', 'Nieuw onderzoek', 'Kies een vragenlijst of onderzoek op maat', 'Start met AI', 'Begin een onderzoeksbrief in een gesprek', 'Vragenlijsteditor', 'Vind uw doelgroep en plan het onderzoek', 'Onderzoeksoverzicht', 'Alleen echte gegevens uit deze werkruimte.', 'Resultaten bekijken', 'Projecten', 'Vragenlijstconcepten', 'In planning', 'Antwoorden', 'Recente projecten', 'Ga verder met lopend onderzoek.', 'Alle projecten', 'Bereid uw eerste onderzoeksbrief voor.'],
  da: ['Start med et forskningsmål, og organiser projekter, spørgeskemaer og ægte svar.', 'Start ny forskning', 'Vælg et skræddersyet spørgeskema eller studie', 'Start med AI', 'Begynd et forskningsbrief i en samtale', 'Spørgeskemaeditor', 'Find din målgruppe og planlæg undersøgelsen', 'Forskningsoversigt', 'Kun ægte poster fra dette arbejdsområde.', 'Se resultater', 'Projekter', 'Spørgeskemakladder', 'Under planlægning', 'Svar', 'Seneste projekter', 'Fortsæt igangværende forskning.', 'Alle projekter', 'Forbered dit første brief.'],
  fi: ['Aloita tutkimustavoitteesta ja järjestä projektit, kyselyt ja aidot vastaukset.', 'Aloita uusi tutkimus', 'Valitse räätälöity kysely tai tutkimus', 'Aloita tekoälyn avulla', 'Aloita tutkimusbrief keskustelussa', 'Kyselyeditori', 'Löydä kohderyhmäsi ja suunnittele tutkimus', 'Tutkimuksen yleiskatsaus', 'Vain tämän työtilan aidot tiedot.', 'Katso tulokset', 'Projektit', 'Kyselyluonnokset', 'Suunnittelussa', 'Vastaukset', 'Viimeisimmät projektit', 'Jatka käynnissä olevaa tutkimusta.', 'Kaikki projektit', 'Valmistele ensimmäinen tutkimusbrief.'],
  no: ['Start med et forskningsmål, og organiser prosjekter, spørreskjemaer og ekte svar.', 'Start ny forskning', 'Velg et tilpasset spørreskjema eller en studie', 'Start med KI', 'Start en forskningsbrief i en samtale', 'Spørreskjemaeditor', 'Finn målgruppen din og planlegg undersøkelsen', 'Forskningsoversikt', 'Bare ekte poster fra dette arbeidsområdet.', 'Se resultater', 'Prosjekter', 'Spørreskjemautkast', 'Under planlegging', 'Svar', 'Nylige prosjekter', 'Fortsett pågående forskning.', 'Alle prosjekter', 'Forbered din første brief.'],
  pt: ['Comece com um objetivo de pesquisa e organize projetos, questionários e respostas reais.', 'Iniciar nova pesquisa', 'Escolher um questionário ou estudo personalizado', 'Começar com IA', 'Iniciar um briefing de pesquisa numa conversa', 'Editor de questionários', 'Encontre o seu público e planeie a pesquisa', 'Visão geral da pesquisa', 'Apenas registos reais deste espaço de trabalho.', 'Ver resultados', 'Projetos', 'Rascunhos de questionários', 'Em planeamento', 'Respostas', 'Projetos recentes', 'Continue a pesquisa em curso.', 'Todos os projetos', 'Prepare o seu primeiro briefing.'],
  ru: ['Начните с исследовательской цели и организуйте проекты, анкеты и реальные ответы.', 'Новое исследование', 'Выберите индивидуальную анкету или исследование', 'Начать с ИИ', 'Создайте исследовательский бриф в диалоге', 'Редактор анкет', 'Найдите аудиторию и спланируйте исследование', 'Обзор исследований', 'Только реальные записи из этого рабочего пространства.', 'Посмотреть результаты', 'Проекты', 'Черновики анкет', 'В планировании', 'Ответы', 'Последние проекты', 'Продолжите незавершённое исследование.', 'Все проекты', 'Подготовьте первый исследовательский бриф.'],
  sv: ['Börja med ett forskningsmål och organisera projekt, enkäter och riktiga svar.', 'Starta ny forskning', 'Välj en anpassad enkät eller studie', 'Börja med AI', 'Starta en forskningsbrief i en konversation', 'Enkätredigerare', 'Hitta din målgrupp och planera undersökningen', 'Forskningsöversikt', 'Endast riktiga poster från den här arbetsytan.', 'Visa resultat', 'Projekt', 'Enkätutkast', 'Under planering', 'Svar', 'Senaste projekt', 'Fortsätt pågående forskning.', 'Alla projekt', 'Förbered din första brief.'],
  tr: ['Bir araştırma hedefiyle başlayın; projeleri, anketleri ve gerçek yanıtları düzenleyin.', 'Yeni araştırma başlat', 'Özel anket veya çalışma seçin', 'Yapay zekâ ile başlayın', 'Bir görüşmede araştırma özeti oluşturun', 'Anket düzenleyici', 'Kitlenizi bulun ve araştırmayı planlayın', 'Araştırma özeti', 'Yalnızca bu çalışma alanındaki gerçek kayıtlar.', 'Sonuçları görüntüle', 'Projeler', 'Anket taslakları', 'Planlanıyor', 'Yanıtlar', 'Son projeler', 'Devam eden araştırmayı sürdürün.', 'Tüm projeler', 'İlk araştırma özetinizi hazırlayın.'],
  ko: ['연구 목표에서 시작해 프로젝트, 설문지와 실제 응답을 정리하세요.', '새 연구 시작', '맞춤 설문 또는 전문 연구 선택', 'AI로 시작', '대화로 연구 브리프 시작', '설문 편집기', '대상자를 찾고 설문 연구를 계획하세요', '연구 개요', '이 작업 공간의 실제 기록만 표시합니다.', '결과 보기', '프로젝트', '설문 초안', '범위 확인 대기', '응답', '최근 프로젝트', '진행 중인 연구를 계속하세요.', '모든 프로젝트', '첫 번째 연구 브리프를 준비하세요.'],
  ja: ['研究目標から始め、プロジェクト、アンケート、実際の回答を整理します。', '新しい研究を開始', 'カスタムアンケートまたは個別研究を選択', 'AIで開始', '対話で研究ブリーフを開始', 'アンケート編集', '対象者を見つけ、アンケート調査を計画', '研究概要', 'このワークスペースの実際の記録のみ表示します。', '結果を見る', 'プロジェクト', 'アンケート下書き', '範囲確認待ち', '回答', '最近のプロジェクト', '進行中の研究を続けます。', 'すべてのプロジェクト', '最初の研究ブリーフを準備しましょう。'],
};
...
export default function BusinessWorkspace() {
  ...
  const workspaceText = workspaceHomeText[language] || workspaceHomeText['en-US'];
      ...
      <div className="business-workspace-body business-workspace-body--rail">
        ...
        {activeView === 'home' ? <section className="business-projects business-workspace-home">
          ...
          <div className="business-dashboard-grid">
            <section className="business-dashboard-statistics"><header><div><h2>{workspaceText[7]}</h2><p>{workspaceText[8]}</p></div><button type="button" onClick={() => selectWorkspaceView('results')}>{workspaceText[9]}</button></header><div className="business-dashboard-stats"><article><span><ClipboardList size={17} /></span><small>{workspaceText[10]}</small><strong>{workspace.projects.length}</strong></article><article><span><Pencil size={17} /></span><small>{workspaceText[11]}</small><strong>{questionnaireDraftCount}</strong></article><article><span><FileText size={17} /></span><small>{workspaceText[12]}</small><strong>{planningCount}</strong></article><article><span><BarChart3 size={17} /></span><small>{workspaceText[13]}</small><strong>{responseCount}</strong></article></div></section>
            <section className="business-dashboard-recent"><header><div><h2>{workspaceText[14]}</h2><p>{workspaceText[15]}</p></div><button type="button" onClick={() => selectWorkspaceView('projects')}>{workspaceText[16]}</button></header>{recentProjects.length ? <div>{recentProjects.map((project) => <button key={project.id} type="button" onClick={() => selectWorkspaceView('projects')}><span className={`business-status status-${String(project.status).toLowerCase()}`}>{projectsCopy.statuses[project.status] || project.status}</span><strong>{project.title}</strong><small>{project.questionnaire?.responseCount || 0} {copy.results.responses}</small><ArrowRight size={15} /></button>)}</div> : <div className="business-dashboard-empty"><ClipboardList size={24} /><strong>{workspaceText[17]}</strong><button type="button" onClick={openNewProject}>{projectsCopy.getStarted}</button></div>}</section>
          </div>
        </section> : activeView === 'questionnaires' ? <section className="business-projects business-questionnaire-hub">
          <div className="business-projects-head"><div><p className="business-eyebrow">{language === 'zh-CN' ? '问卷研究与受众招募' : language === 'zh-Hant' ? '問卷研究與目標對象招募' : language === 'ko' ? '설문 연구 및 대상자 모집' : language === 'ja' ? 'アンケート研究と対象者募集' : 'QUESTIONNAIRE RESEARCH & AUDIENCE RECRUITMENT'}</p><h1>{workspaceText[11]}</h1><p>{language === 'zh-CN' ? '先定义目标受众与研究范围，再编辑问卷题目。系统只创建草稿；不会自动招募、报价或发布。' : language === 'zh-Hant' ? '先定義目標對象與研究範圍，再編輯問卷問題。系統只會建立草稿；不會自動招募、報價或發布。' : language === 'ko' ? '질문을 편집하기 전에 대상자와 연구 범위를 정하세요. 초안만 생성되며 자동 모집, 견적 또는 게시가 진행되지 않습니다.' : language === 'ja' ? '質問を編集する前に対象者と研究範囲を定義します。下書きのみ作成され、自動募集・見積・公開は行いません。' : 'Define the target audience and research scope before editing questions. This creates a draft only; it does not recruit, price, or publish automatically.'}</p></div><button className="business-button" type="button" onClick={() => navigate(withLanguage('/business/audience-plan', language))}><Plus size={16} /> {language === 'zh-CN' ? '规划受众并新建问卷' : language === 'zh-Hant' ? '規劃目標對象並建立問卷' : language === 'ko' ? '대상자 계획 및 설문 만들기' : language === 'ja' ? '対象者を計画してアンケートを作成' : 'Plan audience and create questionnaire'}</button></div>
