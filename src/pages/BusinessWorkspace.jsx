    ...
    <main className="business-workspace">
      <div className="business-workspace-body business-workspace-body--rail">
        ...
        {activeView === 'home' ? <section className="business-projects business-workspace-home">
          ...
          <div className="business-projects-head"><div><p className="business-eyebrow">{language === 'zh-CN' ? '问卷编辑器' : 'QUESTIONNAIRE EDITOR'}</p><h1>{language === 'zh-CN' ? '你的问卷草稿' : 'Your questionnaire drafts'}</h1><p>{language === 'zh-CN' ? '这里仅用于自行创建、编辑和预览问卷。若希望我们协助设计、采集和研究，请从“新建研究”选择定制问卷。' : 'Create, edit, and preview your own questionnaires here. For design, collection, and research support, choose a custom questionnaire from Start new research.'}</p></div><button className="business-button" type="button" onClick={() => setEditorStarterOpen(true)}><Plus size={16} /> {language === 'zh-CN' ? '新建自助问卷' : 'New self-service questionnaire'}</button></div>
          {loading ? <div className="business-workspace-loading"><LoaderCircle className="animate-spin" /> {feedbackCopy.loadingProjects}</div> : questionnaireProjects.length ? <div className="business-questionnaire-hub-list">{questionnaireProjects.map((project) => <article key={project.id}><div><span className={`business-status status-${String(project.questionnaire?.status || project.status).toLowerCase()}`}>{project.questionnaire?.status === 'DRAFT' ? (language === 'zh-CN' ? '问卷草稿' : 'Questionnaire draft') : projectsCopy.statuses[project.status] || project.questionnaire?.status || project.status}</span><p>{project.title}</p><h2>{project.questionnaire?.title || project.title}</h2><small>{language === 'zh-CN' ? `${project.questionnaire?.questionCount || 0} 道题 · ${project.questionnaire?.responseCount || 0} 份答卷` : `${project.questionnaire?.questionCount || 0} questions · ${project.questionnaire?.responseCount || 0} responses`}</small></div><div><Link className="business-project-open" to={withLanguage(`/business/projects/${project.id}`, language)}>{language === 'zh-CN' ? '打开编辑器' : 'Open editor'} <ArrowRight size={15} /></Link><Link className="business-project-open business-project-results" to={withLanguage(`/business/projects/${project.id}/results`, language)}><BarChart3 size={15} /> {projectsCopy.viewResults}</Link>{project.status === 'DRAFT' && <button type="button" className="business-project-open business-project-delete" onClick={() => deleteProject(project)} disabled={submitting}><Trash2 size={15} /> {projectsCopy.deleteDraft}</button>}</div></article>)}</div> : <section className="business-results-index-empty"><ClipboardList size={30} /><h2>{language === 'zh-CN' ? '还没有问卷草稿。' : 'No questionnaire drafts yet.'}</h2><p>{language === 'zh-CN' ? '从一份空白问卷开始；创建后会直接进入题目编辑器。' : 'Start with a blank questionnaire. You will enter the question editor immediately after creating it.'}</p><button className="business-button" type="button" onClick={() => setEditorStarterOpen(true)}><Plus size={16} /> {language === 'zh-CN' ? '新建自助问卷' : 'New self-service questionnaire'}</button></section>}
        </section> : activeView === 'results' ? <section className="business-projects business-results-index">
          <div className="business-projects-head"><div><p className="business-eyebrow">{copy.results.eyebrow}</p><h1>{copy.results.indexTitle}</h1><p>{copy.results.indexIntro}</p></div><div className="business-results-index-actions"><Link className="business-home-project-link" to={withLanguage('/business/delivery-preview', language)}><Eye size={16} /> {language === 'zh-CN' ? '查看交付中心预览' : 'View delivery centre preview'}</Link><button className="business-home-project-link" type="button" onClick={() => selectWorkspaceView('home')}>{copy.rail.services} <ArrowRight size={16} /></button></div></div>
