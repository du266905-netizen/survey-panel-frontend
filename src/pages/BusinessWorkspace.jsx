    ...
    <main className="business-workspace notranslate" translate="no" data-translate="no">
      <div className="business-workspace-body business-workspace-body--rail">
        ...
        {activeView === 'home' ? <section className="business-projects business-workspace-home">
          ...
          <section className="business-workflow-card" aria-label={language === 'zh-CN' ? '项目流程' : 'Project flow'}><div><p>{language === 'zh-CN' ? '同一项目流程' : 'ONE PROJECT FLOW'}</p><h2>{language === 'zh-CN' ? '每一种开始方式，都会回到同一个项目。' : 'Every starting path returns to one project.'}</h2><span>{language === 'zh-CN' ? 'AI 只协助起草；问卷、研究范围与发布都需要你的确认。' : 'AI only helps draft. Questionnaire, scope, and release all need your confirmation.'}</span></div><ol><li><button type="button" onClick={() => navigate(withLanguage('/business/ai-brief', language))}>{language === 'zh-CN' ? 'AI 起草' : 'AI brief'}</button></li><li><button type="button" onClick={() => selectWorkspaceView('questionnaires')}>{language === 'zh-CN' ? '问卷编辑器' : 'Questionnaire editor'}</button></li><li><button type="button" onClick={() => chooseProjectType('research')}>{language === 'zh-CN' ? '定制研究' : 'Managed research'}</button></li><li><strong>{language === 'zh-CN' ? '项目草稿与确认' : 'Project draft & confirmation'}</strong></li><li className="is-pending">{language === 'zh-CN' ? '报价与付款（即将接入）' : 'Quote & funding (coming next)'}</li></ol></section>
        </section> : activeView === 'questionnaires' ? <section className="business-projects business-questionnaire-hub">
          <div className="business-projects-head"><div><p className="business-eyebrow">{language === 'zh-CN' ? '问卷编辑器' : 'QUESTIONNAIRE EDITOR'}</p><h1>{language === 'zh-CN' ? '你的问卷草稿' : 'Your questionnaire drafts'}</h1><p>{language === 'zh-CN' ? 'AI 起草、自建问卷和发送给我们的问卷都在这里继续编辑；发布或请求研究支持前仍需确认。' : 'Continue AI-started, self-built, and submitted questionnaires here. Publishing or requesting research support still requires confirmation.'}</p></div><button className="business-button" type="button" onClick={() => chooseProjectType('questionnaire')}><Plus size={16} /> {language === 'zh-CN' ? '新建问卷' : 'New questionnaire'}</button></div>
          {loading ? <div className="business-workspace-loading"><LoaderCircle className="animate-spin" /> {feedbackCopy.loadingProjects}</div> : questionnaireProjects.length ? <div className="business-questionnaire-hub-list">{questionnaireProjects.map((project) => <article key={project.id}><div><span className={`business-status status-${String(project.questionnaire?.status || project.status).toLowerCase()}`}>{project.questionnaire?.status === 'DRAFT' ? (language === 'zh-CN' ? '问卷草稿' : 'Questionnaire draft') : projectsCopy.statuses[project.status] || project.questionnaire?.status || project.status}</span><p>{project.title}</p><h2>{project.questionnaire?.title || project.title}</h2><small>{language === 'zh-CN' ? `${project.questionnaire?.questionCount || 0} 道题 · ${project.questionnaire?.responseCount || 0} 份答卷` : `${project.questionnaire?.questionCount || 0} questions · ${project.questionnaire?.responseCount || 0} responses`}</small></div><div><Link className="business-project-open" to={withLanguage(`/business/projects/${project.id}`, language)}>{language === 'zh-CN' ? '打开编辑器' : 'Open editor'} <ArrowRight size={15} /></Link><Link className="business-project-open business-project-results" to={withLanguage(`/business/projects/${project.id}/results`, language)}><BarChart3 size={15} /> {projectsCopy.viewResults}</Link></div></article>)}</div> : <section className="business-results-index-empty"><ClipboardList size={30} /><h2>{language === 'zh-CN' ? '还没有问卷草稿。' : 'No questionnaire drafts yet.'}</h2><p>{language === 'zh-CN' ? '从 AI、空白问卷或定制研究需求开始；它们都会被组织成项目。' : 'Start with AI, a blank questionnaire, or a managed research request. Each becomes a project.'}</p><button className="business-button" type="button" onClick={() => chooseProjectType('questionnaire')}><Plus size={16} /> {language === 'zh-CN' ? '新建问卷' : 'New questionnaire'}</button></section>}
